...
int restoration()
{
	...
	inputFile = fopen("LR/ticket.bmp", "rb");
	inputOrg = fopen("ORG/ticket.bmp", "rb");
	outputFile = fopen("output/restoration/ticket.bmp", "wb");
	ORG_Y_File = fopen("ORG_Y/ticket.bmp", "wb");
	...
	double* Y1, * Y2, * Y3, * Y4, * Y_recon, * Y_filter, * Y_org, gx, gy, g,kernel[9] = { 0 }, psnr = 0.0, mse = 0.0;
	...
	Y_filter = (double*)malloc(sizeof(double) * width * height);
	...
	for (int j = 0; j < height; j++)
	{
		...
		{
			Y_filter[j * width + i] = Y_recon[j * width + i] = Y1[j * width + i] = 0.299 * inputImg[j * stride + 3 * i + 2] + 0.587 * inputImg[j * stride + 3 * i + 1] + 0.114 * inputImg[j * stride + 3 * i + 0];
	...
	for (int j = 1; j < height - 1; j++)
	{
		for (int i = 1; i < width - 1; i++)
		{
			Y_filter[j * width + i] = Y_recon[j * width + i] = (Y1[(j - 1) * width + i - 1] + 2 * Y1[(j - 1) * width + i] + Y1[(j - 1) * width + i + 1] + 2 * Y1[(j)*width + i - 1] + 4 * Y1[(j)*width + i] +
	...
	for (int j = 0; j < height; j++)
	{
		for (int i = 0; i < width; i++)
		{
			mse += (double)((Y_org[j * width + i] - Y1[j * width + i]) * (Y_org[j * width + i] - Y1[j * width + i]));
		}
	}
	...
	for (int j = 0; j < height; j++)
	{
		for (int i = 0; i < width; i++)
		{
			mse += (double)((Y_org[j * width + i] - Y_filter[j * width + i]) * (Y_org[j * width + i] - Y_filter[j * width + i]));
		}
	}

	mse /= (width * height);
	psnr = mse != 0.0 ? 10.0 * log10(255 * 255 / mse) : 99.99;
	printf("[가우시안만 적용했을 경우] \nMSE = %.2lf\nPSNR = %.2lf\n\n", mse, psnr);
	mse = 0.0;
	psnr = 0.0;
	for (int j = 0; j < height; j++)
	{
